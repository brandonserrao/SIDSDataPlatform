...
export default {
  ...
  methods: {
    drawChart(parent_selector, options, countryList, pillar, dataFull, metadata) {

      const HALF_PI = Math.PI / 2;
      // TODO: change numberr formating to library based
      function nFormatter(num, digits) {
        ...
        function rankFormat(num) {
        	...
        	else if (number.slice(-1) == 1) { return number.toString() + "st" }
        	else if (number.slice(-1) == 2) { return number.toString() + "nd" }
        	else if (number.slice(-1) == 3) { return number.toString() + "rd" }
        	else { return number.toString() + "th" }
        }


	const data = dataFull[pillar]
...
// console.log("clicked on country ")
  ...
  created(){
    const metaFetch = fetch("https://raw.githubusercontent.com/Ben-Keller/smallislands/main/data/exports/keyMetadata.json");
    const dataFetch = fetch("https://raw.githubusercontent.com/Ben-Keller/smallislands/main/data/exports/allKeyData.json");
    Promise.all([metaFetch, dataFetch]).then(([metaData,data]) => {
      this.metaData = metaData;
      this.data = data;
      console.log(this.metaData);
      console.log(this.data)
      this.drawChart(this.selector, this.options, this.countryList, this.pillar, this.data, this.metaData)
    })
  }
}
</script>
